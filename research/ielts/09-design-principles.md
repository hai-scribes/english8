...
| **B7** | **Vietnamese-specific pronunciation guidance ships only within `07` §8.3's three permitted targets** — coda obstruents incl. final /s~z/ (VN-1), onset-cluster **voicing/aspiration** (VN-2, U6–U7), deaccenting and question tunes (VN-3, U11–U12) — each labelled **[S]**: singly-sourced, panel-unverified, N = 16–36. VN-4's four prohibitions are absolute: **no** Vietnamese-specific vowel guidance (U1–U4), **no** North/South region-specific guidance, **no** justification of word-stress teaching by Vietnamese tone transfer (contested — Brunelle 2017), **no** error prediction by contrasting the two sound inventories. *Added 2026-08-08:* **no claim that coda work raises a score, a band, comprehensibility or intelligibility, and no claim that it transfers to spontaneous speech** — both were tested and neither is evidenced (`07` §5.5.7a–b). VN-1 is a **difficulty** finding, not a payoff finding | A "Vietnamese vowel difficulties" lesson for U1–U4; a Northern-vs-Southern lesson split; "Vietnamese is a tone language, so teach word stress"; presenting the U8 `-s` drill as covering the pronunciation cause too, when `07` §5.5.2 shows the two causes are additive and need two interventions; "final-consonant practice will raise your pronunciation score"; presenting any of it as panel-verified | `07` §5.5, §5.5.7a–c, §8.3; `08` §5.4 |
...
| # | Check | Fails if | Traces to |
| --- | --- | --- | --- |
| **F1** | *(Rewritten 2026-08-08 — the block was lifted, the prohibitions narrowed.)* The ranking function is a **coverage eligibility gate (K1–K4) followed by ranking inside that set on collocational association strength** — `max MI` over the item's collocations, filtered `n > 5`, `t > 2`, tie-break dobj > amod > advmod (`07` §8.1a). **No ranking by raw frequency band, CEFR level, word-list membership or diversity; and no accuracy-scoring module of any kind.** Frequency band, CEFR tag and AWL/AVL flag are stored as attributes only; the MI weighting and the reference corpus stay swappable. Any surface using the ranking states the two caveats: **no intervention study** shows teaching high-MI collocations raises a rating, and **every MI finding is from a B2-and-above population** | An "advanced words first" mode ranked on frequency band; a CEFR-ordered queue; an accuracy scorer; an academic-corpus MI table shipped silently to grade 8 | `07` §3, §8.1a; `07` §3.6 **GAP** ×2 |
...
| **F7** | Items are taught **as collocations, not as words** — the least-contested decision in the area, and the one that survived the 2026-08-08 resolution of the rarity/formulaicity question intact | Bare word–gloss pairs | `07` §8.1, §8.1a; `08` §6.3 item 4 |
...
| --- | --- | --- | --- | --- |
| **1** | **Vocabulary trainer**, CEFR-keyed, fixed-interval review | **Strong.** Technology-assisted L2 vocabulary d = 0.64 (34 studies, N = 2,511) **[V 3-0]**; spacing medium-to-large in L2 (48 experiments, N = 3,411) **[V 3-0]**. *Ranking function added 2026-08-08:* **moderate and correlational** — Saito 2020 (N = 85, bigram MI r = .734/.755, single-word frequency n.s.), Paquot 2019 (98 texts, dobj η² = .154, **C2 cell n = 11**), Kim et al. 2018 — all **[S]** | **High.** Oxford 3000/5000 is CEFR-tagged per word; the expanding-schedule null removes the hardest part of the build. The ranking function needs a **genre-matched** MI table, which is the one real build cost | **Build.** Ranking function **unblocked 2026-08-08**, to `07` §8.1a and no further: coverage gate then `max MI`. Carries two caveats — **no intervention evidence**, and **MI evidence is B2-and-above only** (**F1**) |
...
| **7** | **Pronunciation tooling**, Vietnamese-specific | **Weak but no longer absent.** `07` §5.5 closed on 2026-08-08 with seven sourced studies covering the coda inventory, final /s~z/ omission, onset clusters and question intonation. All **[S]** — quoted, panel-unverified, N = 16–36, several under 20. Vowels, rhythm metrics and region remain **GAP**s | **Medium.** The three permitted targets need recorded audio. *Amended 2026-08-08:* the "coda-consonant lesson the syllabus does not have" is **no longer listed as a build requirement** — the syllabus change was tested against the user's condition and not supported (`07` §5.5.7a–c, `08` §5.4) | **Build, narrowly.** Bounded to `07` §8.3's VN-1/VN-2/VN-3 and bound by VN-4's four prohibitions (**B7**). No pronunciation **band** or score, ever (**D8**); no claim of an intelligibility ranking (`07` §5.5.7 **GAP**); and **no claim of cost or of transfer to spontaneous speech** for coda work (`07` §5.5.7a–b **GAP**s) |
...
| ~~Vocabulary trainer ranking function~~ — **unblocked 2026-08-08**, to one specification | Was `07` §3, §8.1; the rarity-versus-formulaicity question is resolved (`07` §3.0 — word-level and combination-level rarity are different constructs) | **Permitted:** coverage eligibility gate K1–K4, then rank inside that set by `max MI` over the item's collocations, filtered `n > 5`, `t > 2`, tie-break dobj > amod > advmod (`07` §8.1a). Carries `07` §3.6 GAP (i) *no intervention study* and GAP (iv) *MI evidence is B2-and-above only* |
| An **accuracy-scoring module** in the vocabulary trainer, or any ranking by raw frequency band, CEFR level, word-list membership or diversity | `07` §3.5 **GAP** (accuracy is a three-way split, none decisive, error gravity never manipulated); `07` §8.1a's "explicitly NOT supported" table | For accuracy: a study manipulating **error gravity**. For the others: nothing — each was checked against Saito, Paquot and Banerjee and each fails |
| Any claim that **coda / syllable-final consonant work raises a score, band, comprehensibility or intelligibility**, or transfers to spontaneous speech | `07` §5.5.7a **GAP** (no cost evidence for any L1) and §5.5.7b **GAP** (spontaneous-speech CIs cross zero across 77 studies; the one Vietnamese trial is N = 30, uncontrolled, null) | A cost study relating coda-omission rate to band or comprehensibility in Vietnamese B2–C1 speakers, **and** a coda-targeted intervention with a control group and a spontaneous-speech outcome. **Teaching coda contrasts as a difficulty finding is not blocked; claiming a payoff is** |
| Any claim that an **inaudible inflectional ending is charged to the Grammar scale** rather than to Pronunciation | `07` §5.5.7c **GAP** — unexamined by anyone; Kang & Yan's coding makes it true *by construction*, and Isaacs et al. demonstrate bleed in the **opposite** direction | An attribution experiment: resynthesised `-s` within speaker, Pronunciation and Grammar scored separately |
...
band), and Speaking materially under-evidenced.
...
8. **The vocabulary ranking function is no longer among them** — `07` §3 resolved
   ...
   separating **band 8 from band 9** (nearest proxy is **n = 11** C2 texts), and
   what MI values suit **A2/B1** (all MI evidence is B2-and-above). `07` §3.6.
9. **`08` §5.4's coda item is corrected, not carried.** It formerly read as "rank
   1 in the Vietnamese-L1 priority ordering and no unit in the twelve teaches
   them", implying a syllabus request. It is a **difficulty** ranking, not a
   payoff ranking: there is **no cost evidence** (`07` §5.5.7a), **no
   spontaneous-speech trainability evidence** (`07` §5.5.7b), the cross-criterion
   attribution hypothesis is **unexamined** (`07` §5.5.7c), and **age
   appropriateness at ~14 is unknown** (`07` §5.5.7b). The proposal was tested
   against the user's condition and **not supported**; no syllabus change is
   recommended, and the negation is not asserted either.
