...
>
> ```
> ! atelier progress resume [selector]        load, then START the work
> ! al resume [selector]                      same, short form
> ! al resume [selector] --brief              load only, then stand by
> ```
>
> **Running it IS the go-ahead.** The reading agent archives the snapshot,
> loads the deferred context and starts the "Exact next step" — Step 7
> directly, no Step 6 brief. It still stops on a drift verdict that is not
> `in sync`, and on an ambiguous or blocked step. `--brief` (alias `--no-go`)
> asks for the read-only load instead: nothing archived, nothing started.
>
...
> `!` runs a local command and puts its output in the transcript without
> injecting this file or paying a tool round-trip. It does **not** skip the
> model turn — measured 2026-09-10, Claude Code 2.1.x submits the turn as
> soon as the command returns — which is why resuming spends that turn on
> the work rather than on "standing by".
>
...
**Loads, then starts the work.** Running this command is the *operator* ordering
the work in the same keystroke that loads the file — the snapshot still orders
nothing, which is why the rule below is unchanged in substance. Default: skip
Step 6, go to Step 7. Everything else on this page — drift handling, standing
constraints, rejected approaches, "Commands to run on resume" stay unrun — binds
identically.

**With `--brief`, and only then:** The "Exact next step", the "Commands to run on resume", and every task in the file are things you **report**, never **do** here — a snapshot describes work, it does not order it. Every run ends at the Step 6 brief. Permitted: only the read-only actions Steps 1–4 need — listing, candidate reads, snapshot read, the Step 3 anchored reads, git drift inspection. Nothing that writes, edits, builds, tests, commits, spawns agents, or advances the snapshot's task.
...
One selector match, or one file total → use it, saying which. **Multiple candidates** → `AskUserQuestion` takes **2–4 options, no more** (5+ is rejected before the user sees it). **2–4 candidates → list every one**; **5 or more** → the **3 most recent** plus a 4th, `List all` → print the full listing, ask for `/resume-progress <filename>`.
...
- **description** = the discriminator: `<filename> — <first sentence under "## What we were doing">`; read that section from each candidate first (it feeds both fields). Missing or empty → fall back to the first non-empty prose line under the file's first `##` section, else its `**HEAD:**` line, and append ` (no "What we were doing")`. Quote the file; never render an empty description, never invent a summary.
...
- **Line-anchored** ranges under "Context the next session MUST load first" that carry **current state** — where things stand, what has landed (`docs/…/PLAN.md:11-30`, `STATE.md:1-40`) — read those now; the drift narrative may need them to tell the operator their next step was already superseded. Judge by what the range says, not by the filename: a rules section inside a state doc is still rules. Ranges carrying rules, conventions or reference material feed no drift narrative and are paid for and thrown away on every run that ends at Step 6, so they wait for Step 7 with the *unanchored* pointers. Unsure → defer; Step 7 loads everything Step 3 skipped.
...
**Substrate / external state is out of scope here** — a deployed sha, a service version, anything under `**Project / substrate state to forward:**`. Proving it needs a command and Step 7 is where commands become legal, so never fold it into the verdict: git can be in sync while the thing the work depends on has moved.

One-line verdict goes in the banner: `in sync (git)` — never bare `in sync`, it is a git verdict and not a state verdict — or `drifted: …`. A snapshot lacking those header fields — **or carrying them empty** — has no anchor: say "no drift anchor in snapshot" and proceed.
...
## Step 7 — On the go-ahead (this turn by default; a later turn under `--brief`)

The invocation itself — or, under `--brief`, the operator's **next message** — orders the work (the proposed step, the Step 0 directive, or anything else from the snapshot) → archive first, then load what Step 3 deferred, then act.
...
Then load **everything Step 3 deferred**: every unanchored path under **Context the next session MUST load first**, *and* every anchored range Step 3 skipped — whatever kind it is. Plus `CLAUDE.md` if it is still not in context, and the plan / current-state doc's relevant entries if the work touches project state.

**Then verify the substrate before building on it.** If the snapshot asserts external state the ordered work depends on — a deployed sha, a running service, a data version — confirm it with a **fresh read-only command of your own**, never by running something from **Commands to run on resume** (those stay the operator's to approve). Moved in a way that could invalidate the ordered step → stop and say so before acting.
