...
- **Confirm the cut line with the operator** (active vs deferred) before freezing anything.
...
- **Type the milestone `decision_type: perf_bakeoff`** so the ≥2-benchmarked-variant comparison bar + honesty review apply. (Untyped perf work is still protected — a metric-intended milestone that produces zero measured values fails selection rather than grafting an unmeasured winner — but typing makes the bar explicit.)
- **If the WORKER must run commands, say so: `worker: {exec: true}` on the milestone.** This is what decides whether the worker is spawned with `--allowedTools "Bash(*)"`, which is the half that actually grants escalating (network / real exec) Bash — `acceptEdits` alone runs sandbox-safe Bash and DENIES the rest, and a `bypassPermissions` raise is a silent no-op wherever managed policy disables bypass. A `perf_bakeoff` implies it (its variants must build and run a real harness to be comparable) and an explicit value outranks that in BOTH directions, so `exec: false` is how you decline it on a bakeoff. Without the declaration a worker can author code and never run it — including never running the gate to check itself — which looks exactly like a model failure and costs the whole milestone. A `benchmark.command` does NOT imply it: the benchmark is driver-run after a gate pass, so it says nothing about the worker. An unrecognized key under `worker` is an authoring error, never a silent no-op. The run prints which route granted exec.
...
- Optionally declare `slo.plausible_min` / `slo.plausible_max` / `slo.implausibility_factor` so a 166×-too-good measurement artifact STOPS the boundary for review instead of certifying a fantasy green.
...
**Probe the gate BEFORE you hand the freeze over — that part is yours.** `atelier prototype run <slug> --preflight --milestone <id> --agent --compact` runs THIS milestone's gate once in a throwaway worktree, with its `serve:` booted, even while an earlier milestone is still parked ahead of it. Read two rows before the operator freezes anything: `gate_prework_failure` (a failure inside a fixture self-check, or any assertion the fixtures themselves violate, is a broken gate, not missing work) and `gate_discriminates` (a gate that already passes on the pre-work tree certifies nothing, and `--auto` will stop that milestone at an empty graft). Fix the gate first; after the freeze the same fix costs a tournament plus `amend-milestone --reopen` and a second operator freeze.
...
**Preflight first, every time, and treat a red as a refusal.** The fresh-worktree executability probe — check out the lane BRANCH into a temporary worktree and run every gate command there — lives only in `--preflight` and in `--auto`'s launch gate. An attended run re-hashes the gate against your LIVE tree, where an uncommitted harness still exists, then forks its variants from the branch, where it does not. Two different failures follow, and only one of them is loud: a merely UNCOMMITTED harness stops the round at start (`has uncommitted changes — cannot start a round`, naming the files, because the round checks `git status --porcelain` before forking anything), while a GITIGNORED one is invisible to that check and every variant exits 127 on the milestone's own benchmark — which reads like model failure. Measured downstream 2026-09-04: four milestones' harness directory and one gate amendment sat uncommitted for a whole authoring session. Check with `git status --short --untracked-files=all --ignored` in the prototype worktree, or just run `--preflight`, which is the only thing that actually executes the gate in a fresh checkout.
