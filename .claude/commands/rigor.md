...
- **Shapes** — `solo`, `scouts×N` (concurrent product-tree-read-only sub-agents: read seams or staged authoring; product tree = tracked files — `.rigor/` staging sits outside it), `crew×N` (parallel product-tree write leaves under the orchestra machinery, cost-gate-approved, Layer 1 only). There is no "Layer 2": write fan-out is a *how* inside Layer 1's *until-done*.
...
| `atelier rigor procedure fanout` | Crew leaves and staged-generation scouts: always, before dispatch. Read scouts: when the six imperatives aren't enough — a bounded read/verify fan-out may run on them alone, the skip named in the report | the scout brief template, staged-output rules, the measured failures behind the six imperatives |
...
`solo` and `scouts` combine freely with either layer; `crew` is a Layer-1-only shape. Three questions decide it: (1) what would concurrent agents mutate — nothing, their own staging paths, or the product tree? (2) genuinely independent — no piece needs another's output? (3) big enough that overlap beats spawn overhead (each sub-agent pays a fresh cold-cache context)?
...
- **`scouts×N` — product-tree-read-only. Free at both layers; for deep read work fan-out is the DEFAULT.** If your plan contains ≥ 2 genuinely independent read-only seams of real size (each ≳ a few minutes of solo work) **and the seam map is knowable now**, launch them as concurrent sub-agents in a single message — going solo on such work is the marked case and needs a stated reason. When the seam map is *not* knowable yet (you can't partition failures before the baseline run), open `solo→scouts?` with the escalation trigger named in your announcement — honest sequencing, not a downgrade. Scale N to the ask: a quick check is one scout per seam; a **deep/thorough audit crosses seams with concern lenses** (per-file × races/bypass/resume/arithmetic, or whatever the domain's failure classes are) and adds an **adversarial verify pass** — fresh scouts attacking your top findings before they enter the report (discipline #6 in fan-out form). **Pick each scout's model to fit its seam** and state the split. Don't fan out table-stakes work: a task one file-read answers is solo. Partially-coupled reads are still scoutable — brief each on the coupling, then synthesize. **Staged generation rides this shape:** net-new, mechanically checkable artifacts may be authored in parallel by scouts writing only under `<rundir>/<agent-id>/` — freeze the format and a parent-authored checker BEFORE dispatch, have each agent return paths, never contents, then check and promote each unit into the product tree yourself as returns land — where the harness resolves a message's spawns jointly, dispatch in waves and promote between them; batch promotion turns a cap kill into total loss — and close with one integrated check. Cap wave width at 4–6 absent a known concurrency cap, each seam ≳ minutes of real work, else spawn overhead eats the win. Announce `scouts×N(staged)` — the token keys the mandatory fanout-procedure load. No cost gate; imperative #4 is the isolation.
- **`crew×N` — parallel product-tree write leaves. Layer 1 only, and the cost gate decides, not you.** Sub-agents mutating existing product files in parallel need the orchestra machinery (worktrees, frozen interfaces, ledger, integrated verify) — uncoordinated parallel writes are how a green tree silently loses work; a bounded task whose parallel outputs are net-new files is staged scouts, not crew. **Trigger: the moment your Layer-1 plan contains ≥ 2 *independent* product-tree write seams, run the gate — one command, verdict logged.** Independent means disjoint file scopes AND no coherence coupling — files that must read as one surface (an API and its callers) are ONE seam however many files they span, and two write pieces touching the SAME file are never parallel. If the seam map only materializes mid-drive, the trigger fires when it does. Do not pre-judge the gate with mental math; it exists so the break-even is computed, not vibed. The gate prices tokens, never time: where wall-clock is the stated objective, net-new seams of real size default to staged scouts — no gate needed — while crew still launches only on a FANOUT verdict; solo needs a stated reason — coupling, tiny seams, or orchestration overhead. A seam is staged-eligible only if every parallel write is net-new; touches to existing files hoist to the parent's promotion pass.
- **`solo`** — everything else. The one measured forced write-fan-out (n=1, small one-context task, full crew machinery) cost **3.5× the dollars and 2.7× the wall-clock** — a tax of the machinery, not of parallelism; it does not generalize to staged authoring.

## Fan-out bounding and hygiene — the six imperatives

Each was shipped against a *measured* downstream failure, not a hypothetical. Ordinary read scouts may run on these six alone; staged-generation scouts and crew leaves load `atelier rigor procedure fanout` before dispatch, always. Obey these regardless:
...
The parent carries these too — you are the one agent that can see the whole run. Put them **in each scout's brief** (a sub-agent does not inherit this file) and do the cleanup pass at the end of the fan-out.
...
Break-even scales with seam count — `(seams×12k + 8k) / 500` solo-turns: **2 seams → 64 · 3 → 88 · 6 → 160 · 9 → 232**. Estimate marginal solo turns, not isolated ones — shared context amortizes hard (measured 6×). FANOUT needs a seam alone worth `24 + 16/N` un-amortizable turns (two seams → ~32). **SOLO is the near-certain verdict and the gate doing its job**; the logged verdict is your audit evidence.
