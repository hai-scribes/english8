...
hooks:
  Stop:
    - hooks:
        - type: command
          command: "sh -c 'G=\"${ATELIER_HOME_OVERRIDE:-${ATELIER_INSTALL_ROOT:-${ATELIER_HOME:-$HOME/.atelier}}}/bin/rigor-goal-stop-gate\"; [ ! -x \"$G\" ] || exec \"$G\"'"
---
...
- **Layer 1** — evidence-gated persistence: a sealed contract of deterministic conditions plus a check-loop that keeps you working until they actually pass. For a *grind past one context, or past your checks*. If done is not yet mechanized you **author it** — propose it when an operator can answer, seal the evidence floor yourself when nobody can. Machinery loads on demand (Routing).
...
Shape tokens: `solo` · `scouts×N` · `scouts×N(staged)` · `phases×N` · `crew?×N` before the gate, `crew×N` after it — **the `?` is the pre-verdict marker and the only crew form legal before a FANOUT verdict**. Compound shapes join with `→` (`scouts×4→solo`). Contract-status tokens (`eliciting`, `eliciting→sealed`) are *added* to the shape tag, never substituted for it. **At Layer 1 the enforcement token is mandatory:** `hook` = the Stop-hook goal gate is armed for this session, `none` = you drive `check` by hand. **`enforcement: hook` needs two things:** `/rigor` invoked in THIS session process — this file's frontmatter arms the gate; `claude --resume` drops it until `/rigor` is typed again — **and** `atelier rigor stop-gate status` showing `"frontmatter_declared": true` and `"symlink_ok": true`; paste those two fields as the evidence. An unverified `hook` claim is worse than an honest `none`. **`enforcement: none` is an obligation, not a label:** every Layer-1 boundary — before any stop, phase dispatch, or final answer — must show a real **check invocation** in the transcript: the command, the `unmet` ids it returned, and the decisive lines. A prose claim that the conditions pass is not a check. Layer 0 omits the token — the self-test is that layer's gate.
...
```
$ atelier rigor stop-gate status
  "frontmatter_declared": true, "symlink_ok": true   (/rigor invoked in this process)
...
- **Scope inflation** — building more than the ask because more feels safer. The ask's stated coverage IS the contract; extra work is a proposal named in one line, not a deliverable. **Files count, not just work:** a permanent file the ask did not call for (test, script, config, suite-contract test) is scope even when it enforces the ask. An unrequested verification helper lives outside the product tree (`.rigor/`, a temp dir) or is deleted before close. **Scope inflation is about *deliverables* only** — obligations imposed by this discipline or repo policy (the sibling sweep, the verify pass, `CLAUDE.md`'s close-time gates, a mandated continuity read) are part of the ask by definition and never refusable as scope inflation.
...
**Will you be grinding across many turns without a human re-confirming each step?** Judge from *your own plan for the work*, never from how the request was phrased. **The test is verification, not step count or depth:** a horizon is long when you could not settle done at the end with checks you can run — a suite, a count, an oracle you write yourself — or when it exceeds one context.

**"Unattended" means no human re-confirming each step — not "nobody is watching."** It picks the eliciting variant below, never the layer. Nor does depth: an audit, review or read whose deliverable is a report has no mechanical done-state and stays Layer 0, with scouts.

- **Short horizon** → **Layer 0** is the whole command. The common case — do not tax it. **One narrow exception:** readings that lead to *materially different work* and that no single mechanical check of yours would settle either way, with no operator channel → **Layer 1 · `eliciting→sealed`** whatever the horizon, each reading a labeled inferred threshold. Routine judgment calls you **make, state in the announce, and carry at Layer 0**; three of those are not an ambiguity.
- **Long horizon** → **Layer 1.** Layer 0 is no longer available as a default: a long run with no evidence gate ends in a polished report about work that was never done. Load `atelier rigor procedure layer1` before sealing anything. Author 3–6 deterministic done-conditions from the ask if you can; if only partly, that procedure's *Eliciting* section routes you — **never answer "can't mechanize" with a silent fall back to Layer 0.**

**Which eliciting variant** turns on the operator-channel test: *who answers your NEXT message*, never "did a message just arrive." An operator leaving, unreachable, headless, away for days, or handing the work over IS the no-channel case — announce `eliciting→sealed`, seal the evidence floor, itemize your inferred thresholds. Triggers, failure, tie-break: `layer1` § *Eliciting*.

## Shape — the axis is what concurrent agents may mutate

`solo` and `scouts` combine freely with either layer; `phases` and `crew` are Layer-1-only. Three questions decide it: (1) what would other agents mutate — nothing, their own staging paths, or the product tree? (2) genuinely independent — no piece needs another's output? (3) big enough that overlap beats spawn overhead (each sub-agent pays a cold-cache context)?
...
2. **Put the budget in the brief**, with the instruction on hitting it: *return what you have, labeled incomplete*. Every command a scout shells out to gets an explicit, host-verified timeout — **resolve the binary, never assume it**: `TO=$(command -v timeout || command -v gtimeout)` and then `"$TO" 120 …`, because a host that has only `gtimeout` still fails on a bare `timeout`. Stock macOS ships neither, where `timeout 120 …` exits **127 without ever running your command** — and the trap is the next move, not that one: re-running it unwrapped because "the timeout was the problem" is how the unbounded call actually gets made. No binary → bound it from Python (`subprocess.run(..., timeout=120)`), never by dropping the bound. **An idiom you did not verify is a bound you do not have.**
