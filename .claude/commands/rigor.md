...
6. **Is every piece of the ask's stated coverage either done or explicitly named as untouched, with the reason?**
...
**For most tasks Layer 0 plus the two decisions below is all the *ceremony* there is** — decide layer and shape, spend nothing more on process, and run the work itself until the self-test passes (at Layer 1, until the contract does). A done that is not yet mechanized is a contract to elicit, not a reason to stop.
...
**"Do not ask me anything" is ask-suppression and nothing more:** skip `AskUserQuestion`, seal the evidence floor yourself. It is *not* evidence the operator is unreachable for enforcement purposes, and by itself it never justifies skipping a gate, skipping close-out, or degrading the report.
...
- **`phases×N` — sequential delegated write phases, you as orchestrator. Layer 1.** For long multi-feature builds: you hold the sealed contract and never hand it off; each phase goes to a fresh-context sub-agent with a compact brief, hands back through disk artifacts, and you run `check` at the phase boundary before dispatching the next. **A phase is exactly one dispatched writer. A phase that fans out writers is crew and goes to the gate.** Sequential ⇒ one writer at a time ⇒ no cost gate — the crew gate exists for parallel write *loss*, not for delegation. Reach for it on token economics: a long solo drive re-reads its own ever-growing history every turn, while phase workers each keep a small context and the orchestrator keeps only the contract and the returns. Compound freely (`scouts×4→phases×3`, or per-phase read scouts). Mechanics: `layer1` § *Phases*.
...
**On SOLO** — the overwhelmingly common case — keep the contract you already sealed and grind under it, or split it into `phases`, which needs no gate. Collapse the gate's own run so its leftover ledger cannot red `fanout-complete` for a future contract in that workspace: `atelier rigor crew abort --ledger <path> --fanout --reason "solo verdict"`.
