...
async function main() {
     ...
     which figure is lit. */
  {
    ...
    /* ---- every balloon is a drawn contour ---------------------------------
       ...
       the live site with its contour coming and going.

       And ordinary speech — nearly every line in the book — shipped for longer
       than that as a `border-radius` rectangle with a CSS border-triangle stuck
       underneath, which is a label rather than a balloon. There is no radius at
       which a rectangle becomes one, so the fix was not a number: the contour
       is generated as a curve for every kind, and these checks are what stop
       either cheap version coming back. */
    ...
    /* The contour is BUILT, and built out of curves. `ovoidPath` is the entry
       point the balloon geometry hangs off, and it must emit cubic segments —
       a version of it that walked a rounded rectangle in straight lines would
       satisfy every other check on this page while drawing the box back. */
    {
      const fn = APP.match(/function\s+ovoidPath\b[^]*?\n}/);
      ok("comic: the contour is generated by a path builder", !!fn);
      /* Cubic segments, and tangents taken from the neighbouring samples. A
         builder that emitted `L` between the same points would pass every other
         check on this page while drawing the rounded box straight back. */
      ok("comic: and the builder emits curves, not straight edges",
         !!fn && / C"/.test(fn[0]) && /\/ 6\)/.test(fn[0]));
      ok("comic: the words sit in a box that draws nothing",
         /\.d-said\{[^}]*border:0/.test(CSS.replace(/\s+/g, "")));
    }
    {
      ...
      /* The one that used to say the opposite. An ordinary `say` balloon was
         a plain CSS box and this asserted that it stayed one; it is a drawn
         ovoid now, so the assertion is inverted rather than deleted — the rule
         it guards is still "the balloon and the stylesheet agree about what
         draws it", and that rule now has one answer for every kind. */
      const plain = dlg.querySelector('.d-bub[data-bub="say"] .d-said');
      ok("comic: an ordinary balloon is drawn too, not a rounded box",
         !!plain && plain.classList.contains("is-shaped")
         && !!plain.querySelector("svg.d-shape path.d-body-path")
         && !!plain.querySelector("svg.d-shape path.d-tail"));
      while (toShout--) {
        win.dispatchEvent(new win.KeyboardEvent("keydown", { key:"ArrowLeft", bubbles:true }));
        await new Promise(r => setTimeout(r, 30));
      }
