...
Run: python3 tools/make_sheet.py ti
    ...
    in   docs/assets/cast/<slug>/<emotion>.png     six square-ish drawings
    out  docs/assets/cast/<slug>.png               one 3x2 sheet the page loads
...
Three jobs, in order:
  ...
  key       white is flooded out from the border and becomes transparent. It is
            ...
            white", so Thảo's and Khoa's white shirts survive — the style's
            closed black contour is what separates inside from outside
...
CAST_DIR = ROOT / "docs" / "assets" / "cast"
...
def square(im: Image.Image) -> Image.Image:
    """Pad to a square on white. Never crop: a cropped hand is the whole reason
    the panel is square in the first place."""
    w, h = im.size
    if w == h:
        return im
    side = max(w, h)
...
def build(slug: str, emotions: list, cols: int, rows: int, keep_white: bool) -> bool:
    src = CAST_DIR / slug
    if not src.is_dir():
    ...
    for emo in emotions:
        ...
        if not f.is_file():
            ...
            return False
        parts.append((emo, square(Image.open(f).convert("RGB"))))
    ...
    side = max(p.size[0] for _, p in parts)
    sheet = Image.new("RGBA", (side * cols, side * rows), (255, 255, 255, 0))
    for i, (emo, p) in enumerate(parts):
        if p.size[0] != side:
            p = p.resize((side, side), Image.LANCZOS)
        cell = p.convert("RGBA") if keep_white else key_white(p)
...
def main() -> int:
    ...
    for s in slugs:
        if s not in known:
            ...
            continue
        made += build(s, emotions, cols, rows, args.keep_white)
