...
| File | What it holds |
| --- | --- |
| `ROUTER.md` | **Read this first.** How to find one thing without reading the rest |
| `index.jsonl` | One row per section and per named target — the lookup index, generated |
...
| `PROVENANCE.md` | Which PDF this was read from, its checksum, and why it is not committed |

## Do not read this directory wholesale

Seventeen files, ~4,400 lines. Reading them to answer one question wastes a
context window, and an agent that cannot afford to consult the record will write
the lesson from memory instead — which is how the site drifted behind the book
the first time. Grep the index, open the one section it names:

```sh
grep -i 'past continuous' curriculum/sgk/index.jsonl   # -> unit 9, A Closer Look 2
grep '"content-block"' curriculum/sgk/index.jsonl      # every unit's Communication block
grep '"unit": 7,' curriculum/sgk/index.jsonl           # everything in one unit
```

`index.jsonl` is generated by `tools/index_sgk.py` from the headings and
`targets.json`, so it cannot drift from what it indexes. Never hand-edit it;
regenerate it in the same change that edits the record.
